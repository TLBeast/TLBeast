...
export function ScribblesTimeline({ scribbles }: { scribbles: Scribble[] }) {
  ...
  return (
    <div className="mt-12">
      <div className="rounded-xl border border-[var(--color-border-subtle)] bg-[var(--color-bg-elevated)] p-6 sm:p-8">
        <div className="text-center">
          <p className="font-mono text-xs uppercase tracking-widest text-[var(--color-text-subtle)]">
            Slide through your learning journey
          </p>
          ...
          <div className="relative min-w-0 flex-1 pt-6 pb-2">
            ...
            <div className="relative mb-3 h-3">
              ...
              <div className="absolute inset-x-0 top-1/2 flex -translate-y-1/2 justify-between px-0.5">
                {chronological.map((scribble, i) => (
                  <button
                    key={scribble.slug}
                    type="button"
                    onClick={() => selectIndex(i)}
                    aria-label={`${formatDayLabel(scribble.date)} — ${scribble.title}`}
            ...
            <input
              ...
              aria-label="Select scribble by date"
              aria-valuetext={`${formatDate(selected.date)} — ${selected.title}`}
      ...
      >
        <ScribbleContent scribble={selected} />
      </article>
